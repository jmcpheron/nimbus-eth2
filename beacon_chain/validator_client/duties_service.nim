        ...
        for duty in filteredDuties:
          for validatorSyncCommitteeIndex in duty.validator_sync_committee_indices:
            res.add(SyncCommitteeDuty(
              ...
              validator_sync_committee_index: uint16(validatorSyncCommitteeIndex)))
        res
  ...
  let addOrReplaceItems =
    block:
      ...
      for duty in relevantDuties:
        ...
        info "Received new sync committee duty",
             duty = duty,
             epoch = epoch
        res.add((epoch, duty))
      res
    ...
    if sres.isSome():
      for item in addOrReplaceItems:
        ...
        let future = validator.getSyncCommitteeSelectionProof(
          ...
          genesisRoot,
          sres.get(),
          uint64(item.duty.validator_sync_committee_index) div SYNC_SUBCOMMITTEE_SIZE)
        pending.add(future)
        validators.add(validator)
