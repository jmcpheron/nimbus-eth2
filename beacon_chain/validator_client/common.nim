...
type
  ...
  SyncCommitteeDuty* = object
    ...
    validator_sync_committee_index*: uint16
  ...
  for key, item in mpairs(vc.syncCommitteeDuties):
    ...
    item.duties.withValue(epoch, epochDuties):
      ...
      cur.validator_sync_committee_indices.add(
        epochDuties.data.validator_sync_committee_index.IndexInSyncCommittee)
...
proc getSubcommitteeIndex*(syncCommitteeIndex: uint16): SyncSubcommitteeIndex =
  SyncSubcommitteeIndex(uint64(syncCommitteeIndex) div SYNC_SUBCOMMITTEE_SIZE)
